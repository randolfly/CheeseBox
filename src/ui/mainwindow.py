...
class MainWindow(QMainWindow):
    ...
    def show_file_tree(self, file_tree):
        """输入一个文件树，对其进行显示
        ...
        root_file_node = file_tree.root_node
        # self.scene.addFirstFileNode(root_file_node)
        ...
        # ui_root_node.x, ui_root_node.y = m_pos
        # # ui_root_node.num = len(self.scene.NodeList) + 1
        # ui_root_node.setNodeLevel(MainThemeLevel)
        ...
        while stack:
            ...
            for sonNode in v.child_node:
                ...
                # m_pos = self.scene.getSonPos()
                # m_text_name = sonNode.file_name
                # print('m_text_name: ', m_text_name)
                # ui_son_node.x, ui_son_node.y = m_pos
                # ui_son_node.num = len(self.scene.NodeList) + 1
                # if self.scene.m_activateNode.m_level == MainThemeLevel:
                #     ui_son_node.setNodeLevel(SecondThemeLevel)
                # else:
    ...
    def setPropertyDock(self):
        ...
        hotkeyList.addItems(['Ctrl + X 剪切', 'Ctrl + C 复制', 'Ctrl+L 子节点', 'Ctrl+K 并列节点', 'Ctrl+P 打印树', 'Ctrl+Q 退出', 'Ctrl+S 保存树'])
