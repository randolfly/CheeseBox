...
class Graph(QGraphicsScene):
    ...
    def addFirstFileNode(self, fileNode):
        node = self.nodeFactory()
        node.setPos(self.center_x, self.center_y)
        node.setText(fileNode.file_name)
        node.setNodeLevel(MainThemeLevel)
    ...
    def addSonNode(self, text=None):
        ...
        return insertNodeCommand.node


    def addSiblingNode(self,text=None):
        if not self.m_activateNode:
            print('Warning: no activate node !')
            self.messageShow.emit('Warning: no activate node !')
            return
    ...
    def contextMenuEvent(self, e):
        ...
        if selectedItem and not self.m_editingMode:
            ...
            #  New Son Node
            ############################################################################
            new_sonNode_action = QAction('subtopic', self)
            new_sonNode_action.setShortcut('Ctrl+L')
            new_sonNode_action.triggered.connect(self.addSonNode)
            rightclick_menu.addAction(new_sonNode_action)

            ############################################################################
            #  Add Branch
            ############################################################################
            new_siblingNode_action = QAction('topicafter', self)
            new_siblingNode_action.setShortcut('Ctrl+K')
            new_siblingNode_action.triggered.connect(self.addSiblingNode)
            rightclick_menu.addAction(new_siblingNode_action)
